...
def home_window():
    ...
    searchbutton = tk.Button(onscreen, image=search, command=chat)
     ...
     #  font
       ...
       #    color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        # frame containing send button and emoji button
          self.send_button_frame = Frame(self.master, bd=0)
          self.send_button_frame.pack(fill=BOTH)
          self.label =  Button(self.send_button_frame,text = "see questions", command =self.list_insert,activebackground="#FFFFFF",bg='white')
...
def insert():
         ...
         cur.execute(sql,(9,uname,usecase))
         cntn.commit()
