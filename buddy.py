...
Basic_Q = ("who made you", " what is programming"," what is programming?" ," what is programming."," what is programming? ", " ")
...
general = [" Do you know what a theorem is?", " lets first start by this..what's a theorem?"," want to know what a theorem is?"]
general_folow = [" you like therems?", " find that interesting?"," you like it", " did you enjoy the knowlenge" ]
    ...
    for word in sentence.split():
        if word in ANSWERPOS:
           return
            ...
            '''
def linesearch(sentence):
        
       for word in sentence.split():
          if word.lower() in ANSWERNEG:
...
def test(user_response):
    ...
    user_response = user_response[2:]

    if user_response == ans:
        grade = 1
        cur.execute("update dbo.mathquestions set ratings =11 where id = "+str(questn_num))
        return "The Answer You Provided is correct"
    else:
        grade = -1
        cur.execute("update dbo.mathquestions set ratings =-1 where id = "+str(questn_num))
